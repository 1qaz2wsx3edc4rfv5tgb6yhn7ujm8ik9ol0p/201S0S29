...
(select distinct first_value(external_id) over (order by active_date desc, inactive_date desc nulls first) from customer_id_acct_map@cust1 where (account_no = b.account_no or account_no = b.parent_account_no) and external_id_type = 1 and is_current = 1) account, 
...
(
	select 1 from (
		...
		and po.datagrp = 'gsm' and po.bundletype not in ('gsm data', 'mData')
		and exists ( -- invalid cases
		  select 1 from subscriberlist so
		  where 1=1 
		  and so.offer_type = 'so'
		  and po.subscr_no = so.subscr_no
		  and po.subscr_no_resets = so.subscr_no_resets
		  ...
		  where 1=1 
		  and so.offer_type = 'so'
		  and so.subscr_no = po.subscr_no
		  and so.subscr_no_resets = po.subscr_no_resets
		  and po.bundletype = so.bundletype
		  and (
			((so.start_dt = po.start_dt) 
			or (so.start_dt <> po.start_dt and (sysdate-so.start_dt>60 or sysdate-po.start_dt>60)))and
			((so.end_dt = po.end_dt) or (so.end_dt is null and po.end_dt is null))
		  )
		)
