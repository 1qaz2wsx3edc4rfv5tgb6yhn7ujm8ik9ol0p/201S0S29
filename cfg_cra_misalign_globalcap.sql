...
b.offer_id, '"'||f.display_value||'"', f.primary_list_price, e.foreign_code, e.offer_type, b.offer_inst_id, b.chg_who, to_char(b.chg_dt, 'dd-Mon-yyyy hh24:mi:ss') chg_dt, to_char(b.active_dt, 'dd-Mon-yyyy hh24:mi:ss') active_dt, to_char(b.inactive_dt, 'dd-Mon-yyyy hh24:mi:ss') inactive_dt,
...
and b.subscr_no in (
select oiv.subscr_no from offer_inst_view@cust1 oiv, offer_ref@cust1 oref, reseller_version@cust1 rv
where oiv.offer_id = oref.offer_id and rv.reseller_id = 7 and rv.inactive_date is null and rv.status in (3) and oref.reseller_version_id = rv.reseller_version_id and oref.offer_type = 2 and oref.equip_type_code = 101 and oiv.view_status = 2 and oiv.inactive_dt is null and oiv.active_dt > to_date('20150101','yyyymmdd')
and exists (select 1 from offer_inst_view@cust1 a where a.view_status = 2 and a.offer_id in (3720,3721,3722) and a.inactive_dt is null and a.active_dt > oiv.active_dt and a.subscr_no = oiv.subscr_no) -- Align Global Cap
)
and (b.offer_id in (3720,3721,3722) or e.offer_type =2) -- GSM Non Data and Global Cap
order by a.account_no, b.subscr_no, f.primary_list_price, e.offer_type asc, active_dt desc, inactive_dt desc nulls first, offer_id
;