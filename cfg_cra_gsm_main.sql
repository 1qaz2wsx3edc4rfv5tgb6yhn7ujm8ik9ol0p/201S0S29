...
) where customer_account_id = account and service_id = service)) NO_GPRS, */
...
(
select 1 from (
select distinct so.subscr_no from subscriberlist so
where 1=1
and so.offer_type = 'so' and so.bundletype in ('gsm data','mData')
and so.datagrp = '4g'
and exists ( -- invalid po-so dates
    select 1 from subscriberlist po
    where po.offer_type = 'po'
    and po.bundletype = so.bundletype
    and po.subscr_no = so.subscr_no and po.subscr_no_resets = so.subscr_no_resets
    and (
      po.start_dt <> so.start_dt 
      --or (po.end_dt <> so.end_dt and po.end_dt is not null and so.end_dt is not null)
      or (po.end_dt <> so.end_dt and po.end_dt is not null and so.end_dt is not null)
      or (so.end_dt is null and po.end_dt is not null) 
      or (so.end_dt is not null and po.end_dt is null)
    )
    and not exists ( -- valid po-so dates
      select 1 from subscriberlist sl 
      where sl.bundletype = po.bundletype and sl.offer_type = 'so' 
      and sl.subscr_no = po.subscr_no and sl.subscr_no_resets = po.subscr_no_resets
      and (sl.start_dt = po.start_dt or (sl.start_dt <> po.start_dt and (sysdate - sl.start_dt > 60 or sysdate - po.start_dt > 60)))
      --and (sl.end_dt = po.end_dt or (po.end_dt is null and sl.end_dt is null)))
      and (sl.end_dt = po.end_dt or (po.end_dt is null and sl.end_dt is null))
    )
    and not exists ( -- valid so-vas dates
      select 1 from subscriberlist vas
      where vas.bundletype = 'vas'
      and vas.subscr_no = so.subscr_no and vas.subscr_no_resets = so.subscr_no_resets
      and (vas.start_dt = so.start_dt or (vas.start_dt <> so.start_dt and (sysdate - vas.start_dt > 60 or sysdate - so.start_dt > 60)))
      --and (sl.end_dt = po.end_dt or (po.end_dt is null and sl.end_dt is null)))
      and (vas.end_dt = so.end_dt or (so.end_dt is null and vas.end_dt is null))
    )
    and not exists ( -- cbp to surfshare where discounts are disconnected next day but cbp started immediately
      select 1 from subscriberlist po, subscriberlist oldpo
      where po.datagrp = 'surfshare' and oldpo.datagrp <> 'surfshare'
      and po.subscr_no = oldpo.subscr_no and po.start_dt = oldpo.end_dt
      and oldpo.bundletype = so.bundletype
      and oldpo.end_dt+1 = so.end_dt
    )
